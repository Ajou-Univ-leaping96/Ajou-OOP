...
public class IntegralTest {
	public static double integral(F f, double a, double b, int n) { //f인 함수를 a에서 b까지 n개로 쪼개서 적분하는 함수
		...
		for (i = 0; i < n; i++) {
			...
			//apply 메소드는  double형 인자 한개를 가지고 double형 값을 리턴하도록 정해진다.
		}
		return sum * dt;
	}
	...
	public static void main(String[] args) {
		F f;
		double r;
		f = (x) -> {return (x);}; //f 변수의 인터페이스의 apply에 람다식으로 함수를 구현하는 모습 => "f(x) = x" 로 변경하였다!
		
		//f = x ->  x  ; //축약버전
		
		r = integral(f, 0, 10, 100); //apply가 구현된 f를 가지고 구분구적법으로 적분
		System.out.printf("f(x)=x, 0, 10, 100: %.5f%n", r); //소숫점 다섯자리까지 표현  => "f(x) = x" 로 변경하였다!
	}
}