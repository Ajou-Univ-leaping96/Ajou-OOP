...
public class IntegralTest {
	...
	public static void main(String[] args) {
		F f;
		double r;
		f = (x) -> {return (Math.sin(x));}; //f 변수의 인터페이스의 apply에 람다식으로 함수를 구현하는 모습 
		
		//f = x ->  sin(x) ; //축약버전
		
		r = integral(f, 0, Math.PI, 10000); //apply가 구현된 f를 가지고 구분구적법으로 적분 =>n을 100에서 10000으로 변경
		System.out.printf("sin(x)=x, 0, PI, 10000: %.5f%n", r); //소숫점 다섯자리까지 표현 =>10000개로 쪼개서 적분한 결과 소수점 5자리까지는 모두 0으로 표시됨
	}
}